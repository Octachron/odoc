...
let add_extension_constructor identifier (ec:Component.Extension.Constructor.t) env =
    ...
    List.fold_left
      (fun env orig ->
        ...
        | Odoc_model.Lang.Signature.ModuleTypeSubstitution t ->
            let ty = module_type empty
                { id = t.id; doc = t.doc; expr = Some t.manifest; canonical = None  } in
